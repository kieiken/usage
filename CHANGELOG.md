...
- `NOTICE.md` を追加し、オリジナル作者 lollapalooza 氏、元プロジェクト、AGPL-3.0-only の帰属情報を記録。
- Release installer で `usage.app` と Claude Desktop 使用量推算 LaunchAgent をまとめてインストールできるようにした。
- ダブルクリックでインストールできる `usage-installer.pkg` を作成する `scripts/build_pkg.sh` を追加。
- `usage_client.py` は Claude Desktop 推算ファイル `~/.claude/usage-status-derived.json` を最優先で読み取る。
