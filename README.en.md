...
`usage` is a macOS menu bar tool that pins your **Claude Desktop, Claude Code, and Codex** usage to the top-right of your screen. Click the icon for a popover showing Session, Weekly, per-project usage (today / 7-day / monthly), and today's token usage and cost estimate.
...
This distribution is a modified version of the original `usage` by lollapalooza, adapted so Claude Desktop usage can be estimated from local logs instead of relying only on the Claude Code CLI statusLine hook. See [NOTICE.md](NOTICE.md).
...
1. `~/.claude/usage-status-derived.json` — estimated from local Claude Desktop logs by the release installer.
2. `~/.claude/usage-status.json` — written by the hook usage installs.
3. `~/.claude/usag-status.json` — automatic v0.1.x legacy fallback; new users should not encounter this.
4. `~/.claude/tt-status.json` — fallback. If you also use [token-tracker](https://github.com/stormzhang/token-tracker), usage will share its status file.
...
### First launch: optional Claude Code hook

The release installer already installs the Claude Desktop usage deriver. If you also want Claude Code CLI statusLine data, click the "フックをインストール" button in the popover. Then **fully quit Claude Code (Cmd+Q) and re-open it**, click "Refresh now" in usage, and the numbers will appear.
...
> Using the .app? Click the "フックをインストール" button in the popover if you need Claude Code CLI data. The steps below are for developers running usage from source.
...
- usage only reads `~/.claude/usage-status-derived.json`, `~/.claude/usage-status.json`, the v0.1.x legacy `~/.claude/usag-status.json`, `~/.claude/tt-status.json`, Claude Desktop / Claude Code local logs, and Codex's session files. It does not call the Anthropic / OpenAI API and does not read the Keychain. The only network activity is a one-time download of the LiteLLM pricing table for Codex cost estimates (cached for 7 days; offline fallback available).
...
|---------|--------------|-----|
| Menu bar shows `--` | No status JSON has been generated yet | **.app users**: confirm the installer ran, open Claude Desktop once, then click Refresh. **Source users**: run `python3 main.py --setup` if you need Claude Code CLI data |
...
This distribution keeps attribution to the original author, lollapalooza. If you fork or redistribute a modified version, keep the original copyright notice, AGPL-3.0-only license, and project link:
