...
- **Read priority** in `usage_client.py`: `usage-status-derived.json` (Claude Desktop deriver) → `usage-status.json` → `usag-status.json` (v0.1.x legacy) → `tt-status.json` (token-tracker compatibility fallback).
...
- The primary README is Japanese and `README.en.md` is English. Preserve original attribution in `NOTICE.md` and keep CHANGELOG changes mirrored between `.md` and `.en.md` when practical.
